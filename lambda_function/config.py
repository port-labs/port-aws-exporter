...
def _get_resources_config(event, lambda_context):
    ...
    assert (
        next_config_file_key != original_config_file_key
    ), "next_config_file_key must not equal CONFIG_JSON_FILE_KEY"

    config_json_file_key = next_config_file_key or original_config_file_key
    config_from_s3 = json.loads(
        aws_s3_client.get_object(Bucket=bucket_name, Key=config_json_file_key)[
            "Body"
        ].read()
    )

    assert (
        "resources" in config_from_s3
    ), "resources key is missing from config file json"
    ...
    if next_config_file_key:  # In case it's a re-invoked lambda
        ...
        try:
            ...
            logger.warning(
                f"Failed to clean config state, bucket: {bucket_name}, key: {next_config_file_key}; {e}"
            )
    else:
        next_config_file_key = os.path.join(
            os.path.dirname(original_config_file_key),
    ...
    s3_config = {
        ...
        "next_config_file_key": next_config_file_key,
    }
